...
/**
 * Get all available bills in a reduce way.
 * Only id, name, billId
 */
app.get("/small/all", function(req, res, next) {

  api.bill.smallAll(function (err, billDocs) {
    if (!billDocs)
      return res.send(404);
    log('Serving Reduce Bill %s', billDocs.billId);
    res.json(billDocs);
  });
