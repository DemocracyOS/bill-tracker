...
WorkflowView.prototype.loadSteps = function (billId) {
  ...
  request.get(billUrl).end(function (res) {
    ...
    //Load first text to show in the first load.
    if (dom('#bill-diff .text').text() === '') {
      //TODO: this should trigger the click event, not only add the text.
      self.emit('step load', steps[0].billText);
    }
  });
};
